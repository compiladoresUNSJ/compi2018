...
namespace at.jku.ssw.cc
{
    ...
    class Parser
    {
        ...
        static async void Program()
        {
            ...
            {
                Code.Colorear("latoken"); Parser.MessageBoxCon3Preg();//muestro y paro //si existiera una declaracion, as "int i", colorea "int";  (yaPintado = true)
            ...
            {
                MethodDecl(methodDeclsOpc);  //void Main() int x,i; {val = new Table;....} // cuelga y pinta todos los datos internos al main 
            }
        ...
        static void MethodDecl(System.Windows.Forms.TreeNode methodDeclsopc)
        {
            ...
            if (la == Token.VOID || la == Token.IDENT)
            {
                ...
                if (la == Token.IDENT)
                {
                    ...
                    Code.Colorear("token"); MessageBoxCon3Preg();// muestro y paro //pinta el ")"
                    methodDecl.Nodes.Add("')'");
                    MessageBoxCon3Preg(); // muestro y paro
                    Check(Token.RPAR);
                }
                else
                {
                    ...
                    Code.Colorear("latoken"); MessageBoxCon3Preg();// muestro y paro  //pinta el ")"
                    Check(Token.RPAR);
                    Code.seleccLaProdEnLaGram(8);
                    pars.Nodes.Add(".");
                    pars.ExpandAll();
                ...
                //void Main()==> int x,i; {val = new Table;....}
                {
                    ...
                    {
                        //encuentraDecl = true;
                        Code.Colorear("latoken"); MessageBoxCon3Preg();// muestro y paro //colorea "int"  en int i; 
                        ...
                        MessageBoxCon3Preg(); // muestro y paro
                        Code.seleccLaProdEnLaGram(6);
                        VardDecl(Symbol.Kinds.Local, varDecl); // int x,i; en MethodDecl()  con int ya consumido  //cuelga mas datos y pinta
                    }
                    else
                    {
                        token = laToken;
        ...
        static void Block(System.Windows.Forms.TreeNode methodDecl)
        {
            ...
            methodDecl.ExpandAll();
            MessageBoxCon3Preg(block); //muestro y paro
            ////// Agrega '{' al arbol
            Check(Token.LBRACE);
            block.Nodes.Add("'{'");
            block.ExpandAll();
            MessageBoxCon3Preg(methodDecl); //muestro y paro
            Code.Colorear("token");
            ...
            if (la == Token.RBRACE)
            {
                ...
                MessageBoxCon3Preg(statementsopc); //muestro y paro
            }
            int ii = 1;
            while (la != Token.RBRACE)
            {
                  ...
                  || la == Token.LBRACE || la == Token.SEMICOLON) && la != Token.EOF)
                {
                    ...
                    statementsopc.ExpandAll();
                    MessageBoxCon3Preg(statement); //muestro y paro
                    Code.seleccLaProdEnLaGram(18);
                    if (ZZ.ParserStatem)
                    {
